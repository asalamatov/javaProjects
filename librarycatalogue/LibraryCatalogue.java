...
public class LibraryCatalogue {
    ...
    public Book getBook(String bookTitile){
        return getBookCollection().get(bookTitile);
    }
    ...
    public void returnBook(String title){
        Book book  = getBook(title);
        int daysLate = currentDay - (book.getDateCheckOut() + getLengthOfCheckoutPeriod());
        if (daysLate > 0){
    ...
    public void sorryBookAlreadyCheckedOut(Book book){
        ...
        + "It should be back on day " + (book.getDateCheckOut() + getLengthOfCheckoutPeriod()));
    }

    
    public static void main(String[] args){
        Map<String, Book> bookCollection = new HashMap<String, Book>();
        Book harry = new Book("Harry Potter", 8354, 999420);
